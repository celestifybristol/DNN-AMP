...
namespace Risdall.Modules.DNN_AMP.Components
{
    ...
    {
        // https://stackoverflow.com/questions/30054517/get-image-dimensions-directly-from-url-in-c-sharp
        ...
        {
            // check cache before attempting some potentially slow processes
            var cachekey = uri.AbsoluteUri.ToAlphaNumeric();
            var cache = System.Web.HttpRuntime.Cache;

            if (cache[cachekey] != null)
            {
                return (Size)cache[cachekey];
            }
            ...
            while (moreBytes)
            {
                try
                {
                    ...
                    var d = GetDimensions(new BinaryReader(new MemoryStream(allBytes)));
                    cache[cachekey] = d;
                    return d;
                }
                catch
            ...
            // headers didn't work, download the entire stupid thing
            try
            {
                byte[] imageData = new WebClient().DownloadData(uri.AbsoluteUri);
                MemoryStream imgStream = new MemoryStream(imageData);
                Image img = Image.FromStream(imgStream);
                var dimensions = new Size(img.Width, img.Height);
            ...
            var emptydimensions = new Size(0, 0);
            cache[cachekey] = emptydimensions;
